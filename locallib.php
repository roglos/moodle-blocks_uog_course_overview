...
/**
 * Helper functions for uog_course_overview block
 *
 * @package    block_uog_course_overview
 ...
 */
function block_uog_course_overview_get_overviews($courses) {
    $htmlarray = array();
...
/**
 * Sets user preference for maximum courses to be displayed in uog_course_overview block
 *
 * @param int $number maximum courses which should be visible
 */
function block_uog_course_overview_update_mynumber($number) {
    set_user_preference('uog_course_overview_number_of_courses', $number);
}

/**
 * Sets user course sorting preference in uog_course_overview block
 *
 * @param array $sortorder list of course ids
 */
function block_uog_course_overview_update_myorder($sortorder) {
    ...
    set_user_preference('uog_course_overview_course_sortorder', $value);
}

/**
 * Gets user course sorting preference in uog_course_overview block
 *
 * @return array list of course ids
 */
function block_uog_course_overview_get_myorder() {
    if ($value = get_user_preferences('uog_course_overview_course_sortorder')) {
        return explode(',', $value);
    }
    // If preference was not found, look in the old location and convert if found.
    $order = array();
    if ($value = get_user_preferences('uog_course_overview_course_order')) {
        $order = unserialize($value);
        block_uog_course_overview_update_myorder($order);
        unset_user_preference('uog_course_overview_course_order');
    }
    return $order;
}
 ...
 */
function block_uog_course_overview_get_child_shortnames($courseid) {
    global $DB;
    ...
    $sql = "SELECT c.id, c.shortname, $ctxselect
            ...
            WHERE e.courseid = :courseid AND e.enrol = :method AND ctx.contextlevel = :contextlevel ORDER BY c.shortname ASC";
    ...
    if ($results = $DB->get_records_sql($sql, $params)) {
        ...
        if ($total > 10) {
            ...
            if ($diff > 1) {
                $suffix = get_string('shortnamesufixprural', 'block_uog_course_overview', $diff);
            } else {
                $suffix = get_string('shortnamesufixsingular', 'block_uog_course_overview', $diff);
            }
        }
        $shortnames = get_string('shortnameprefix', 'block_uog_course_overview', implode('; ', $shortnames));
        $shortnames .= $suffix;
    }
...
/**
 * Returns maximum number of courses which will be displayed in uog_course_overview block
 *
 * @param bool $showallcourses if set true all courses will be visible.
 * @return int maximum number of courses
 */
function block_uog_course_overview_get_max_user_courses($showallcourses = false) {
    ...
    if (empty($config->forcedefaultmaxcourses)) {
        if ($showallcourses) {
            ...
            $limit = get_user_preferences('uog_course_overview_number_of_courses', $limit);
        }
    }
    return $limit;
}
 ...
 */
function block_uog_course_overview_get_sorted_courses($showallcourses = false) {
    global $USER;

    $limit = block_uog_course_overview_get_max_user_courses($showallcourses);
    $courses = enrol_get_my_courses($fields = NULL, $sort = 'visible DESC,shortname ASC', $limit = 0);
    $site = get_site();
    ...
    $order = block_uog_course_overview_get_myorder();
