 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class block_uog_course_overview_renderer extends plugin_renderer_base {

    /**
     * Construct contents of uog_course_overview block
     *
    ...
    public function uog_course_overview($courses, $overviews) {
        ...
        if ($ismovingcourse) {
            ...
            $html .= get_string('movingcourse', 'block_uog_course_overview', $a);
        ...
        foreach ($courses as $key => $course) {
            ...
            if (!empty($config->showchildren) && ($course->id > 0)) {
                // List children here.
                if ($children = block_uog_course_overview_get_child_shortnames($course->id)) {
    ...
    public function editing_bar_head($max = 0) {
        ...
        $options = array('0' => get_string('alwaysshowall', 'block_uog_course_overview'));
        for ($i = 1; $i <= $max; $i++) {
            $options[$i] = $i;
        }
        $url = new moodle_url('/my/index.php');
        $select = new single_select($url, 'mynumber', $options, block_uog_course_overview_get_max_user_courses(), array());
        $select->set_label(get_string('numtodisplay', 'block_uog_course_overview'));
    ...
    public function hidden_courses($total) {
        ...
        if (!empty($config->forcedefaultmaxcourses)) {
            $output .= get_string('hiddencoursecount'.$plural, 'block_uog_course_overview', $total);
        } else {
            $a = new stdClass();
            $a->coursecount = $total;
